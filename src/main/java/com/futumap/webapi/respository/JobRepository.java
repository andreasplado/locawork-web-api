...
public interface JobRepository extends JpaRepository<JobEntity, Integer> {
    ...
    @Query(value="SELECT * from jobs j WHERE earth_box(ll_to_earth(?1,?2),?3) @> ll_to_earth(j.latitude,j.longitude) AND j.account_google_id !=?4 AND regitred_account=''", nativeQuery = true)
    ...
    @Query(value="SELECT * from jobs j WHERE earth_box(ll_to_earth(?1,?2),?3) @> ll_to_earth(j.latitude,j.longitude) AND regitred_account=''", nativeQuery = true)
