...
public interface JobApplicationRepository extends JpaRepository<JobApplicationEntity, Integer> {
    ...
    @Query(value="SELECT new com.futumap.webap.dto.JobApplicationDTO(ja.id, ja.is_approved, j.title, j.description, j.salary, ja.created_at, ja.updated_at, ja.job_id, ja.user_id) FROM job_applications ja" +
